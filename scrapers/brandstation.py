...
class Scraper(Base_Scraper):
    def __init__(self):
        super().__init__("Brandstation")

    def scrape(self, useFile: bool):
        adress = "https://astar.se/restaurang-brandstation-lunchmeny/"
        if useFile:
            text = util.cached_request(adress, "brandstation")
        else:
...
class _Parser(HTMLParser):
    ...
    def handle_endtag(self, tag):
        if self.found_day and tag == "strong":
            self.in_day_div = True
            self.found_day = False
        if self.in_day_div and tag == "p":
            self.in_day_div = False
